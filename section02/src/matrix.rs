use crate::vector::{dot_product, MyVector};
    ...
    T: Copy + Default + Add<Output = T> + AddAssign + Mul<Output = T> + Send + 'static,
{
    ...
    for i in 0..a.row {
        for j in 0..b.col {
            let row = MyVector::new(&a.data[i * a.col..(i + 1) * a.col]);
            let col_data = b.data[j..]
                .iter()
                .step_by(b.col)
                .copied()
                .collect::<Vec<_>>();
            let col = MyVector::new(col_data);
            let idx = i * b.col + j;
...
// 为实现了Display的T, 自定义实现Display Trait, 输出格式
...
impl<T> MsgInput<T> {
    pub fn new(idx: usize, row: MyVector<T>, col: MyVector<T>) -> Self {
        Self { idx, row, col }
    }
}
